...
class ExpenseClaimApprovalWorkflow(Document):
	pass
...
def populate_approval_workflow(doc, method):
	if ("Administrator" not in frappe.get_roles(frappe.session.user)):
		# If doc.approval_status in 'Draft', populate workflow. Else, check against created workflow
		if doc.approval_status == 'Draft':
			doctype = doc.doctype
			company = doc.company
			grand_total = doc.total_claimed_amount
			...
			for d in doc.approval_workflow:
				...
				if d.status == 'Pending':
					...
					if frappe.session.user == d.user:
						if doc.approval_status in d.possible_actions:
							# If thats the result_state, mark Completed, else still Pending
							if doc.approval_status == d.result_state:
								d.status = "Completed"
								d.state = doc.approval_status
								return
							else:
								d.status = "Pending"
								d.state = doc.approval_status
				...
				# if status is 'Completed'
					if (i+1) < len(approvals):
						...
						if doc.approval_status == 'Applied':
							doc.exp_approver = approvals[i+1].user
						return
					else:
						doc.to_notify_next = ''
						return