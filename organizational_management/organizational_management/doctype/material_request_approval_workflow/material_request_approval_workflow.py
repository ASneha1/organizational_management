...
class MaterialRequestApprovalWorkflow(Document):
	pass
...
def populate_approval_workflow(doc, method):
	if ("Administrator" not in frappe.get_roles(frappe.session.user)):
		# If doc.workflow_state in 'Draft', populate workflow. Else, check against created workflow
		if doc.workflow_state == 'Draft' and doc.material_request_type == 'Purchase':
			doctype = doc.doctype
			company = doc.company
			grand_total = 0
			...
			# Identify workflow by - Doctype, Company and enable_approval_workflow
			...
			if doc.docstatus==0 and doc.material_request_type == 'Purchase':
				...
				for d in doc.approval_workflow:
					...
					if d.status == 'Pending':
						...
						if frappe.session.user == d.user:
							if doc.workflow_state in d.possible_actions:
								# If thats the result_state, mark Completed, else still Pending
								if doc.workflow_state == d.result_state:
									d.status = "Completed"
									d.state = doc.workflow_state
									return
								else:
									d.status = "Pending"
									d.state = doc.workflow_state
					...
					for d in doc.approval_workflow:
						if frappe.session.user == d.user:
							if doc.workflow_state in d.possible_actions:
								...
								frappe.throw(_("No Permission to perform the action"))
					if ("Purchase User" not in frappe.get_roles(frappe.session.user)) and ("Purchase Manager" not in frappe.get_roles(frappe.session.user)):
						frappe.throw(_("Only Purchase User or Purchase Manager can submit Material Request of type Purchase."))
			if doc.docstatus==1:
				if doc.material_request_type == 'Purchase':
					if ("Purchase User" not in frappe.get_roles(frappe.session.user)) and ("Purchase Manager" not in frappe.get_roles(frappe.session.user)):
						frappe.throw(_("Only Purchase User or Purchase Manager can submit Material Request of type Purchase."))
				else:
					if ("Stock User" not in frappe.get_roles(frappe.session.user)) and ("Stock Manager" not in frappe.get_roles(frappe.session.user)):
						frappe.throw(_("Only Stock User or Stock Manager can submit Material Request of types other than Purchase."))
